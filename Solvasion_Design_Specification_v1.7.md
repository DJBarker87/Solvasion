# Solvasion — Design Specification v1.7

## Fully On-Chain Territory Conquest on Solana

**Author:** Dom Barker
**Date:** March 2026
**Status:** Draft (revised from v1.6 incorporating theatre model correction to event-driven grants, RegionMap account elimination, multiplier stacking cap, capture bonus bot-farming prevention, and attack refund threshold)
...
- **RegionMap account eliminated** — Region assignment simplified to trusted admin parameter on `claim_hex`, verified by `region_map_hash` on Season account.
...
**Capture bonus:** On capture via `reveal_defence` (attacker wins), if the defender revealed a non-zero defence (`energy_amount > 0`), the attacker receives `season.capture_bonus_points` (default 50) added to their `points` field.

**Not awarded on timeout:** Capture bonus is NOT awarded on `resolve_timeout`. Uncontested captures don't earn bonus points.

**Bot farming prevention:** The bonus only applies when the defender revealed non-zero defence. This prevents farming by capturing weakly-held bot territory or AFK players' hexes.
...
**Refund on defender win:** `refund = attack.energy_committed * season.attack_refund_bps / 10000` (default 2500 = 25%). Added to attacker's `energy_balance`.
...
| Attack energy | Defender reveals | Outcome | Energy returned |
|---|---|---|---|
| 10 (minimum probe) | 15 (defender wins) | Below threshold -- no refund | 0 |
| 30 (serious probe) | 40 (defender wins) | Above threshold -- 25% refund | 7 |
| 80 (major assault) | 100 (defender wins) | Above threshold -- 25% refund | 20 |
...
**Region assignment at claim time:** The `claim_hex` instruction accepts a `region_id: u8` parameter stored on the Hex account. The region assignment is determined by the map generation script based on the hex's geographic centre coordinates. The frontend passes the correct `region_id` from the published mapping.

**Region map verification:** The Season account stores a `region_map_hash: [u8; 32]` -- the SHA256 hash of the canonical sorted `(hex_id, region_id)` mapping, set during season creation. Anyone can independently verify correctness by recomputing the hash. The program does not validate individual `region_id` values against a lookup at claim time -- the admin is trusted for season configuration, consistent with the existing trust model for landmarks and map data.
...
Active theatres are stored on the Season account and updated by the admin via `set_active_theatres`. The crank calls this every 48 hours, selecting 2-3 regions weighted toward contested regions, avoiding consecutive repeats.
...
| `region_map_hash` | `[u8; 32]` | SHA256 hash of canonical (hex_id, region_id) mapping |
...
| `region_id` | `u8` | Region this hex belongs to (set at claim time via trusted admin parameter) |
...
**Parameters:** `season_id`, `hex_id`, `adjacent_hex_id` (if required), `adjacency_chunk_index: u8` (if adjacency required), `initial_commitment: [u8; 32]`, `initial_nonce: u64`, `region_id: u8`
...
- Set hex.region_id = region_id (trusted admin parameter)
...
- **Resolve combat:**
    - If attack.energy_committed > energy_amount: **Attacker wins**
        ...
        - If energy_amount > 0: add season.capture_bonus_points to player_attacker.points
