# Solvasion — Design Specification v1.6

## Fully On-Chain Territory Conquest on Solana

**Author:** Dom Barker
**Date:** March 2026
**Status:** Draft (revised from v1.5 incorporating mandatory bluff commitments, named regions, war feed, frontline visualisation, theatre objectives, narrative bots, daily briefing, contracts/bounties, retaliation token, posture system, two-step escalation with landmark decisiveness, capture bonus points, and attack refund)
...
**Theatre multiplier interaction:** During Stage 2, if a landmark is also in an active theatre, both multipliers apply multiplicatively. A landmark in an active theatre during Stage 2 generates: `5 × 5 (landmark escalation) × 2 (theatre) = 50 points/hour`.
...
Every successful hex capture awards a flat on-chain point bonus to the attacker, creating a direct incentive to attack that pure territory-holding cannot replicate.

**Capture bonus:** On capture (attacker wins via `reveal_defence` or `resolve_timeout`), the attacker receives `season.capture_bonus_points` (default 50) added to their `points` field.

**Daily capture bonus:** The first capture within any rolling 24-hour window earns an additional `season.daily_capture_bonus_points` (default 100). Tracked via `player.next_capture_bonus_at` timestamp. The rolling window is timezone-free — every player has their own cycle tied to when they last earned it.

**Anti-stalemate effect:** A turtle holding 40 hexes earns ~960 points/day from territory. An aggressive player holding 25 hexes but capturing one hex per day earns ~600 + 150 = 750 points/day — much closer than the territory gap suggests, and the attacker is also disrupting the turtle's territory.

#### 2.4.9 Attack Refund

When an attacker loses combat (defender reveals and wins), a percentage of the attacker's committed energy is refunded, reducing the cost of probing enemy defences.

**Refund on defender win:** `refund = attack.energy_committed × season.attack_refund_bps / 10000` (default 2500 = 25%). Added to attacker's `energy_balance`. Refund only applies when the defender actively reveals and wins — not on timeout or attacker wins.

**Design rationale:** A 25% refund changes the expected value of attacking. An attacker committing 80 energy to a probe now risks 60 energy (not 80) if they lose. Combined with the information gained from the reveal, the failed attack becomes an intelligence-gathering investment rather than a pure loss. This creates a healthy "probing meta."
...
Theatre Objectives are rotating regional point multipliers that create temporary geographic focal points for conflict. Every 48 hours, the admin (via crank) designates 2–3 named regions as active theatres. Hexes within an active theatre earn bonus on-chain points during the theatre window.

Theatre Objectives directly affect the on-chain points that determine the season winner. A player who ignores theatres and turtles in a non-theatre region generates fewer points than one who fights over theatre hexes.

#### 2.8.2 Region Definitions

The map is divided into named regions. Each region is identified by a `region_id` (u8, 1–15). Every hex belongs to exactly one region. The hex-to-region mapping is stored on-chain in a **RegionMap** account, uploaded during season setup alongside the ValidHexSet and AdjacencySet data.
...
Active theatres are stored on the Season account and updated by the admin via `set_active_theatres`. The crank calls this every 48 hours, selecting 2–3 regions weighted toward regions with active frontlines and away from regions dominated by a single player. The same region cannot be selected in consecutive windows.

#### 2.8.4 Theatre Point Multiplier

The theatre multiplier is applied during lazy points calculation. Hexes in active theatres earn `theatre_point_multiplier_bps` (default 20000 = 2×) times normal points. Player accounts track `theatre_hex_count` and `theatre_landmark_count` for efficient calculation.

#### 2.8.5 Anti-Stalemate Design Rationale

Without on-chain theatre multipliers, a player holding 40 well-defended hexes in a quiet region generates steady points toward victory. With 2× theatre points, a player with 20 hexes in an active theatre generates the same points as one with 40 outside it. Theatres rotate every 48 hours, so no region is permanently advantaged — but the player who adapts to rotations will reach victory faster. This forces movement, creates conflict at predictable geographic points, and rewards aggressive, adaptive play.
...
| `theatre_point_multiplier_bps` | `u16` | Point multiplier for theatre hexes (default 20000 = 2×) |
| `capture_bonus_points` | `u32` | Points awarded per successful capture (default 50) |
| `daily_capture_bonus_points` | `u32` | Additional points for first capture in 24h window (default 100) |
| `attack_refund_bps` | `u16` | Attack energy refund on failed attack, basis points (default 2500 = 25%) |
...
| `theatre_hex_count` | `u32` | Number of owned hexes in active theatre regions |
| `theatre_landmark_count` | `u8` | Number of owned landmark hexes in active theatre regions |
| `next_capture_bonus_at` | `i64` | Timestamp after which next daily capture bonus is available (0 = immediately) |
...
| `region_id` | `u8` | Region this hex belongs to (set at claim time via RegionMap lookup) |
...
**PDA seed:** `["region_map", season_id.to_le_bytes(), chunk_index]`

Stores sorted array of hex-to-region mappings. Created during season setup alongside ValidHexSet and AdjacencySet.

| Field | Type | Description |
|-------|------|-------------|
| `season_id` | `u64` | Season |
| `entry_count` | `u32` | Number of entries in this chunk |
| `entries` | `[(u64, u8); N]` | Sorted array of (hex_id, region_id) pairs |
...
- Set hex.region_id via RegionMap binary search lookup
...
- **Resolve combat:**
    - If attack.energy_committed > energy_amount: **Attacker wins**
        ...
        - If now >= player_attacker.next_capture_bonus_at: add season.daily_capture_bonus_points to player_attacker.points, set player_attacker.next_capture_bonus_at = now + 86400
    - If attack.energy_committed ≤ energy_amount: **Defender wins**
        ...
        - Calculate refund: refund = attack.energy_committed × season.attack_refund_bps / 10000
        - Add refund to player_attacker.energy_balance (capped at energy_cap)
...
- Verify RegionMap account exists and contains entries
- Verify RegionMap entry count matches ValidHexSet hex count
...
#### 3.3.29 `init_region_map`

**Purpose:** Create RegionMap account for a season. Admin only.

**Accounts:** admin (signer, payer), season (read), region_map (PDA, init), system_program

**Logic:**
- Verify caller is admin
- Verify map_finalized is false
- Create RegionMap account with season_id

#### 3.3.30 `append_region_data`

**Purpose:** Write chunks of hex-to-region mappings. Admin only.

**Accounts:** admin (signer), season (read), region_map (mut)

**Parameters:** `entries: Vec<(u64, u8)>`

**Logic:**
- Verify caller is admin
- Verify map_finalized is false
- Append entries to region_map.entries
- Update entry_count
...
| `AttackResolved` | season_id, attack_id, result, attacker, defender, hex_id, attacker_refund |
...
| `TheatreActivated` | season_id, theatre_regions, expires_at, multiplier_bps |
| `CaptureBonusAwarded` | season_id, player, hex_id, base_bonus, daily_bonus_granted |
...
| `RegionMapMissing` | `finalize_map_data` | RegionMap account does not exist |
| `RegionMapIncomplete` | `finalize_map_data` | RegionMap entry count does not match hex count |
...
6. Upload hex data on-chain via `init_valid_hexes` + `append_hex_data` (chunked by transaction size)
7. Upload adjacency data via `init_adjacency` + `append_adjacency_data` (chunked)
8. Upload region data via `init_region_map` + `append_region_data` (chunked)
9. Call `finalize_map_data` to lock the data and enable gameplay
...
CREATE TABLE theatres (
    ...
    point_multiplier_bps INTEGER NOT NULL DEFAULT 20000,
