# Solvasion — Design Specification v1.7.2

## Fully On-Chain Territory Conquest on Solana

**Author:** Dom Barker
**Date:** March 2026
**Status:** Draft (revised from v1.7 incorporating v1.7.1 and v1.7.2 errata: region Merkle proof validation, theatre expiry invariants, capture bonus gating simplification, attack refund arithmetic precision)
...
**Region assignment at claim time:** The `claim_hex` instruction accepts a `region_id: u8` parameter and a Merkle proof of `(hex_id, region_id)` membership. The proof is verified against `season.region_map_root` stored on the Season account. This prevents malicious clients from claiming hexes with incorrect region IDs to farm theatre bonuses.

**Region map Merkle tree:** The map generation script constructs a Merkle tree from the region mapping. Leaves are `SHA256(hex_id.to_le_bytes() || [region_id])`, sorted by `hex_id` ascending. The root is stored on-chain as `season.region_map_root`, set during season creation and immutable for the season's lifetime. The script also outputs proof data (sibling hashes + direction flags bitfield) for the frontend. At ~500 hexes, proof depth is ~9 levels (288 bytes + 2-byte flags), well within transaction size limits.
...
| `region_map_root` | `[u8; 32]` | Merkle root of canonical sorted (hex_id, region_id) mapping |
...
**Parameters:** `season_id`, `hex_id`, `adjacent_hex_id` (if required), `adjacency_chunk_index: u8` (if adjacency required), `initial_commitment: [u8; 32]`, `initial_nonce: u64`, `region_id: u8`, `region_proof: Vec<[u8; 32]>`, `region_proof_flags: u16`
...
- Compute leaf = SHA256(hex_id.to_le_bytes() || [region_id])
- Verify Merkle proof: walk region_proof using region_proof_flags for left/right ordering, confirm result equals season.region_map_root
- If verification fails, reject with `InvalidRegionProof` error
...
| `InvalidRegionProof` | `claim_hex` | Merkle proof verification failed for (hex_id, region_id) |
